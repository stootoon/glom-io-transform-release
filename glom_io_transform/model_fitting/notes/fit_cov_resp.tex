...
$$ \ZZ_\text{free} = (\XX \XX^T + \lambda \II)^{-1} (\YY \XX^T + \lambda \II).$$

When we constrain them to be symmetric, we add a Lagrange multiplier to the loss, to get $$ \loss{sym} = \loss{resp} + \tr(\bLa^T (\ZZ - \ZZ^T)).$$
...
If we define $$ \bSig_{xx} \triangleq \XX \XX^T + \lambda \II, \quad \bSig_{xy} \triangleq \XX \YY^T + \lambda \II, \quad \bSig_{yx} \triangleq \YY \XX^T + \lambda \II,$$ we get $$ \bSig_{xy} + \bSig_{yx} = \ZZ \bSig_{xx} + \bSig_{xx}\ZZ.$$

$\bSig){xx}$ is invertible, so we get equivalently$$ \bSig_{xx}^{-1} \bSig_{xy} + \bSig_{xx}^{-1} \bSig_{yx} = \bSig_{xx}^{-1}\ZZ \bSig_{xx} + \ZZ.$$ Recognizing the second term on the left-hand side as the free solution, gives a relation between the two as $$ \bSig_{xx}^{-1} \bSig_{xy} + \ZZ_\text{free} = \bSig_{xx}^{-1}\ZZ_\text{sym} \bSig_{xx} + \ZZ_\text{sym}.$$ Rearranging, we can express $\ZZ_\text{free}$ in terms of $\ZZ_\text{sym}$ as $$ \ZZ_\text{free} = \bSig_{xx}^{-1}(\ZZ_\text{sym} \bSig_{xx} - \bSig_{xy}) + \ZZ_\text{sym}.$$  
$$\blacksquare$$
\end{document}
