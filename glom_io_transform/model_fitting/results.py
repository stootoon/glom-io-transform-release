...
class ModelResults:
    ...
    def extract(self, seed=0, train=0, metric=None, la = None, with_params =False, **kwargs):
        ...
        # The noise floor used to be assembled here from split.test/split.vld.
        # It now lives in compute.output_noise_floor, which draws its own
        # trial-disjoint pairs: only one vld draw is stored per split, so a
        # second independent estimate cannot be formed from these results.
