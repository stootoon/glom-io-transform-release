...
def group_order(models, prefix, df=None):
    ...
    has_output = "Output" in cols and (
        df is None or (cols["Output"] in df.columns and df[cols["Output"]].notna().any()))
...
def violin_data(df, sampler, mode, prefix="corr", outclass=None, models=None,
    ...
    panel += [ViolinPlotData(values(m, cols["est"]), colour_for(m), models[m])
              for m in names]
    # All-NaN means this split's sampler cannot define a floor; see panel_units.
    # Checked on THIS panel's rows: a frame holding several splits will have a
    # floor for the trials ones and none for the odours ones.
    if ("Output" in cols and cols["Output"] in df.columns
            and df.loc[rows, cols["Output"]].notna().any()):
...
def panel_brackets(df, prefix, sampler, mode, comparisons, outclass=None,
    ...
    panel_rows = (df["sampler"] == sampler) & (df["mode"] == mode)
    if outclass is not None:
        panel_rows &= (df["outclass"] == outclass)
    order = group_order(models, prefix, df[panel_rows])
