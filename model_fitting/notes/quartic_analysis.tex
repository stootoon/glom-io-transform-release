...
\subsubsection*{$g<0$, $u^2 \le 1$}
\begin{itemize}
\item Here $K$ is real, so $u$ is real.
\item Since $|u| \le 1$, $\arccosh(u) = j \arccos(u)$.
\item Then $s_k = K \cosh(j(\arccos(u) + 2 \pi k)/3) = K \cos{\arccos(u) + 2\pi k \over 3}.$
\item Three real roots, one will be the saddle.
