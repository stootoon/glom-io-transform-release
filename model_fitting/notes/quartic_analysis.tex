...
To interpret this term, we consider its numerator. Assuming $N$ is large enough that we can take $\alpha \approx 1$ for simplicity, we can write the numerator as \begin{align*} N^2 q_i + p_i m - 2a_i^2 &= N^2 \xx_i^T \EE_i \xx_i + \|\xx_i\|_2^2 \|\mm_i\|_2^2 - 2 (\xx_i^T \mm_i)^2 \\
  &=  \|\xx_i\|_2^2 \left( N^2{\xx_i^T \EE_i \xx_i \over \|\xx_i\|_2^2} + \|\mm_i\|_2^2 - 2 {(\xx_i^T \mm_i)^2 \over \|\xx_i\|_2^2} \right) \\
  &= \|\xx_i\|_2^2 (N^2 \hat \xx_i^T \EE_i \hat \xx_i + \|\mm_i\|_2^2 - 2 (\hat \xx_i^T \mm_i)^2)\\ &= N^2 \|\xx_i\|_2^2 \left(\hat \xx_i^T \EE_i \hat \xx_i + {\|\mm_i^\perp\|_2^2 - \| \mm^{||}_i\|_2^2 \over N^2}\right)\\
  &=N^2 \|\xx_i\|_2^2  \left(\hat \xx_i^T \EE_i \hat \xx_i + {\|\mm_i\|_2^2(1 - 2 \cos(\theta_i)^2) \over N^2}\right)\\
  &= N^2 \|\xx_i\|_2^2  \left(\hat \xx_i^T \EE_i \hat \xx_i - {\|\mm_i\|_2^2 \cos(2 \theta_i) \over N^2}\right).
\end{align*}
where we've defined $$ \mm_i^{||} \triangleq \mm_i^T \hat \xx_i \hat \xx_i, \quad \mm_i^\perp \triangleq \mm_i - \mm_i^{||}$$ as the components of $\mm_i$ parallel and perpendicular to $\xx_i$. Therefore, $$ \boxed{ \|\xx_i\|^2 g = \|\xx_i\|^2 {\tilde c \over 2} =  \hat \xx_i^T \EE_i \hat \xx_i - {\|\mm_i\|_2^2 \cos(2 \theta_i) \over N^2}.}$$  
...
To interpret this quantity we again consider it's numerator, and assume $N$ is large enough that we can take $\alpha \approx 1$. We then get
\begin{align*}
  N^2 (q_i a_i -b_i p_i) + a_i p_i m - a_i^3 &= N^2 \left((\xx_i^T \EE_i \xx_i) (\xx_i^T \mm_i) - (\xx_i^T \EE_i \mm_i) (\xx_i^T \xx_i) + {(\xx_i^T \mm_i) (\xx_i^T \xx_i) (\mm_i^T \mm_i) - (\xx_i^T \mm_i)^3 \over N^2}\right)\\
  &= N^2 \|\xx_i\|^3 \|\mm_i\|\left( (\hat \xx_i^T \EE_i \hat \xx_i)(\hat \xx_i^T \hat \mm_i) - \hat \xx_i^T \EE_i \hat \mm_i + {\hat \xx_i^T \hat \mm_i \|\mm_i\|^2 - (\hat \xx_i^T \hat \mm_i)^3 \|\mm_i\|^2 \over N^2}\right)\\
  &= N^2 \|\xx_i\|^3 \|\mm_i\| \left( \hat \xx_i^T \EE_i \hat \xx_i \cos(\theta_i) - \hat \xx_i^T \EE_i \hat \mm_i + \cos(\theta_i){\|\mm_i\|^2 - \cos(\theta_i)^2 \|\mm_i\|^2 \over N^2}\right)\\
  &= -N^2 \|\xx_i\|^3 \|\mm_i\|\left(\hat \xx_i^T \EE_i \hat \mm^\perp_i \sin(\theta_i) - {\|\mm_i\|^2 \sin(\theta_i)^2\cos(\theta_i)\over N^2}\right)\\
  &= -N^2 \|\xx_i\|^3 \|\mm^\perp_i\|\left(\hat \xx_i^T \EE_i \hat \mm^\perp_i - {\|\mm_i\|^2 \sin(2 \theta_i)\over 2 N^2}\right)
\end{align*}
where $\hat \mm_i^\perp$ is the unit vector along the component of $\mm_i$ perpendicular to $\xx_i$. Therefore
$$ \boxed{\|\xx_i\|^3 h = \|\xx_i\|^3 {\tilde d \over 4} = -{1 \over N}   \|\mm^\perp_i\| \left(\hat \xx_i^T \EE_i \hat \mm^\perp_i - {\|\mm_i\|^2 \sin(2 \theta_i)\over 2 N^2}\right).}$$
We can also express this entirely in terms of $\mm_i^\perp$ as $$ \|\xx_i\|^3 h =  -{1 \over N} \|\mm^\perp_i\| \left(\hat \xx_i^T \EE_i \hat \mm^\perp_i - {\|\mm^\perp_i\|^2 \cot(\theta_i)\over N^2}\right).$$  
...
To find the roots $s$, take wolog $W = u + \sqrt{u^2 - 1}$.
